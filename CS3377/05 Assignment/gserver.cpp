/**
 * Guess Word gserver file
 * Winston Shih
 * 4/10/2024
 * Reads into a vector<string> from a file name provided by argv[1] to set a word as word for user to guess.
 */
#include <iostream>
#include <fstream>
#include <string>
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        {
            stringstream s;
            s<<"Error opening file Words File "<<argv[1]<<endl;
        ...
        if(forkpid==CHILD_PID)
        {
            ...
            stringstream outss;
            outss<<i;
            srd_cwr_np_str=srd_cwr_np_str+(outss.str());
